...
int32_t m_addfriend_with_create_time(Messenger *m, const uint8_t *address, const uint8_t *data, uint16_t length, uint64_t create_time)
{
    if (length > MAX_FRIEND_REQUEST_DATA_SIZE) {
        return FAERR_TOOLONG;
    }
...
static State_Load_Status friends_list_load(Messenger *m, const uint8_t *data, uint32_t length)
{
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (temp.status >= 3) {
            ...
            net_to_host(create_time, sizeof(uint64_t));
