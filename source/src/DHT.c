...
static int handle_sendnodes_ipv6(void *object, IP_Port source, const uint8_t *packet, uint16_t length, void *userdata)
{
    ...
    for (uint32_t i = 0; i < num_nodes; ++i) {
		//in.s_addr = plain_nodes[i].ip_port.ip.ip.v4.uint32;
		//printf("ip:%s---port:%d\n", inet_ntoa(in), ntohs(plain_nodes[i].ip_port.port));
...
void dht_save(const DHT *dht, uint8_t *data)
{
    ...
    clients = (Node_format*)malloc(sizeof(Node_format)*MAX_SAVED_DHT_NODES);
    if(clients == NULL)
    {
        DEBUG_PRINT(DEBUG_LEVEL_INFO,"dht_save:malloc failed");
    }
    uint32_t num = 0;
