    ...
    if (msglen >= MAX_CRYPTO_DATA_SIZE) 
    {
        ...
        while (1) 
        {
            ...
            if (!tmsg) {
                ...
                free(StrFrame);
                free(RspStrParams);
                DEBUG_PRINT(DEBUG_LEVEL_ERROR, "cfd_tox_send_message calloc failed");
                break;
            }
...
int cfd_usersend_textmessage(int mlist_id,struct lws_cache_msg_struct * pmsg)
{
    ...
    if(to_uid <= 0)
    {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"get to_id(%s) failed",to_idstring);
        return ERROR;
    }
	...
	int* plws_index, struct imcmd_msghead_struct *head)
{
    ...
    {
        //旧版本用toxid作为用户id
        uid_len = strlen(user_toxid);
        if(uid_len == TOX_ID_STR_LEN)
        {
            cfd_olduseridstr_getbytoxid(user_toxid,uid_str);
            if(strlen(uid_str) != CFD_USER_PUBKEYLEN)
            {
                DEBUG_PRINT(DEBUG_LEVEL_ERROR,"cfd_userlogin_deal:bad input toxid(%s)",user_toxid);
        ...
        {
            //sign 验证
            if(pnr_sign_check(sign,strlen(sign),p_uidstr,TRUE) != OK)
            {
                ret_code = PNR_LOGIN_RETCODE_BAD_LOGINKEY;
                DEBUG_PRINT(DEBUG_LEVEL_ERROR,"cfd_userlogin_deal:input sign(%s) usrkey(%s) check failed",sign,p_uidstr);
            }
            cfd_uinfolistgetdbid_byuidstr(p_uidstr,&uid);
            if(uid <= 0)
            {
                ret_code = PNR_LOGIN_RETCODE_BAD_UID;
                DEBUG_PRINT(DEBUG_LEVEL_ERROR,"cfd_userlogin_deal:input usrkey(%s) check failed",p_uidstr);
            }
            else
            {
