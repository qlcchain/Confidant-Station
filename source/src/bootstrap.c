...
#ifdef DEV_ONESPACE
#define DEFAULT_NODES_FILENAME "/root/pnrouter/DHTnodes.json"
#else
#define DEFAULT_NODES_FILENAME "/usr/pnrouter/DHTnodes.json"
#endif
...
static int curl_fetch_nodes_JSON(struct Recv_Curl_Data *recv_data)
{
    ...
    if (ret != CURLE_OK) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "TLSv1.2 could not be set (libcurl error %d)", ret);
        goto on_exit;
    }
    ...
    if (ret != CURLE_OK) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "Failed to set TLS cipher list (libcurl error %d)", ret);
        goto on_exit;
    }
    */
...
static bool nodeslist_needs_update(const char *nodes_path)
{
    ...
    bool is_timeout = timed_out(last_scan, 7 * 24 * 60 * 60);
...
{
    FILE *fp = NULL;
    char line[MAX_NODELIST_SIZE + 1] = {0};

    if (!file_exists(DEFAULT_NODES_FILENAME)) {
        if ((fp = fopen(DEFAULT_NODES_FILENAME, "w+")) == NULL) {
            DEBUG_PRINT(DEBUG_LEVEL_ERROR, "nodeslist load error: failed to create "
                "file '%s'\n", DEFAULT_NODES_FILENAME);
            goto on_exit;
        }
    } else if ((fp = fopen(DEFAULT_NODES_FILENAME, "r+")) == NULL) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "nodeslist load error: failed to open "
            "file '%s'\n", DEFAULT_NODES_FILENAME);
        goto on_exit;
    }

    int update_err = update_DHT_nodeslist(DEFAULT_NODES_FILENAME);
    if (update_err < 0) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "update_DHT_nodeslist() failed "
            "with error %d\n", update_err);
    }
    
    if (fgets(line, sizeof(line), fp) == NULL) {
        fclose(fp);
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "nodeslist load error: file empty.\n");
        goto on_exit;
    }
    ...
    if (Nodes.count == 0) {
        ...
        fclose(fp);
        DEBUG_PRINT(DEBUG_LEVEL_ERROR, "nodeslist load error: List did not contain any valid entries.\n");
        goto on_exit;
    }
